...
def init_weights(net, name='normal', gain=0.02):
    def init_func(m):
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
            if name == 'normal':
                ...
                raise NotImplementedError('initialization method [%s] is not implemented' % name)
    ...
    print('Init type: {}, gain: {}'.format(type, gain))
    net.apply(init_func)


def init_net(net, init_name='normal', init_gain=0.02, ngpu=1, **kwargs):
    logger.info('------------ {} ----------'.format(str(net.__class__.__name__)))
    init_weights(net(**kwargs), name=init_name, gain=init_gain)
