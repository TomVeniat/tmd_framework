...
def init_corruption(name, **kwargs):
    if name == 'keep_patch':
        return KeepPatch(**kwargs)
    elif name == 'remove_pix':
        return RemovePixel(**kwargs)
    elif name == 'remove_pix_dark':
        return RemovePixelDark(**kwargs)
    elif name == 'conv_noise':
...
class KeepPatch(object):
    ...
    def measure(self, x, theta=None, seed=None):
        assert (self.im_size is not None)
        x_measured = x.clone()

        if theta is None:
            mask = self.sample_theta(im_shape=x.shape, seed=seed)
            mask = torch.tensor(mask, dtype=torch.uint8, requires_grad=False)
        else:
            mask = theta
